...
ВАЖНО ДЛЯ 'calendar':
- Для каждого события указывай ТОЧНУЮ дату в формате YYYY-MM-DD.
- Если на скриншоте видно несколько дней (например, неделя с Пн по Вс), каждое событие должно иметь дату того дня, на котором оно стоит.
- Если год не указан явно, используй {current_year}.
...
{{
  ...
  "data": {{
    ...
    "events": [
      {{"title": "название", "date": "YYYY-MM-DD", "time": "HH:MM"}}
    ]
  }}
}}
"""
